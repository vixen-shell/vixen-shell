...
class Commands:
    ...
    def env_dependencies(
        env_dir: str, requirements_filepath: str, destination: str = None
    ):
        target = "" if not destination else f" --target {destination}"
        return f"source {env_dir}/bin/activate && pip install --no-cache-dir --upgrade pip && pip install --no-cache-dir -r {requirements_filepath}{target}"
