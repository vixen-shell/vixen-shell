...
def get_service_name_and_object_path(service: str) -> tuple[str, str]:
    index = service.find("/")
...
class StatusNotifierHostInterface(object):
    def __init__(self, host_id, trays: typing.List[Tray]):
        ...
        self.host_service_name = HOST_SERVICE_NAME_TEMPLATE.format(
        ...
        self.watcher_service_observer = DBusObserver(
            message_bus=self.session_bus, service_name=WATCHER_SERVICE_NAME
        )
    ...
    def watcher_available_handler(self, _observer):
        ...
        self.watcher_proxy.StatusNotifierItemRegistered.connect(
            self.item_registered_handler
        )
        self.watcher_proxy.StatusNotifierItemUnregistered.connect(
            self.item_unregistered_handler
        )
        self.watcher_proxy.RegisterStatusNotifierHost(
            self.host_object_path, callback=lambda _: None
        )
    ...
    def item_registered_handler(self, full_service_service):
        ...
        service_name, object_path = get_service_name_and_object_path(
            full_service_service
        )
    ...
    def find_item(
        self, service_name, object_path
    ) -> typing.Union[StatusNotifierItem, None]:
